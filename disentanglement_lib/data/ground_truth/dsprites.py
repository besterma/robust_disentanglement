...
class NoisyDSprites(DSprites):
    ...
    def sample_observations_from_factors(self, factors, random_state):
        ...
        return np.minimum(observations + color, 1.0).astype(np.float32)
...
class ScreamDSprites(DSprites):
    ...
    def __init__(self, latent_factor_indices=None):
        ...
        with gfile.Open(SCREAM_PATH, "rb") as f:
            ...
            self.scream = np.array(scream) * 1.0 / 255.0
    ...
    def sample_observations_from_factors(self, factors, random_state):
        ...
        for i in range(observations.shape[0]):
            ...
            background = (
                self.scream[x_crop : x_crop + 64, y_crop : y_crop + 64]
                + random_state.uniform(0, 1, size=3)
            ) / 2.0
