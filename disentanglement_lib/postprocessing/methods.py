...
def sampled_representation(
    ground_truth_data, gaussian_encoder, random_state, save_path
):
    ...
    def transform_fn(mean, logvar):
        return tf.add(mean, tf.exp(logvar / 2) * tf.random_normal(tf.shape(mean), 0, 1))
