...
def postprocess_with_gin(
    model_dir, output_dir, overwrite=False, gin_config_files=None, gin_bindings=None
):
...
@gin.configurable("postprocess", blacklist=["model_dir", "output_dir", "overwrite"])
def postprocess(
    model_dir,
    output_dir,
    overwrite=False,
    ...
    with hub.eval_function_for_module(module_path) as f:
        ...
        transform_fn, transform_checkpoint_path = postprocess_fn(
            dataset, _gaussian_encoder, np.random.RandomState(random_seed), output_dir
        )
    ...
    results.update_result_directory(
        results_dir, "postprocess", results_dict, original_results_dir
    )
