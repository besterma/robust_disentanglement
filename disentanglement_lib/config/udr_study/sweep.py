...
def get_datasets():
    ...
    nr_channels_udrvae = h.sweep(
        "vae_quant.UDRVAE.num_channels", h.categorical([1, 3, 3, 3, 1, 3, 3])
    )
    nr_channels_vae = h.sweep(
        "vae_quant.VAE.num_channels", h.categorical([1, 3, 3, 3, 1, 3, 3])
    )
...
class UdrStudy(study.Study):
    ...
    def get_eval_config_files(self):
        ...
        return list(resources.get_files_in_folder("config/udr_study/metric_configs/"))
