...
def _make_symmetric_psd(matrix):
    return 0.5 * (matrix + matrix.T) + np.diag(np.ones(10)) * 10.0


class VaeTest(parameterized.TestCase, tf.test.TestCase):
    @parameterized.parameters(
        (np.zeros([10, 10]), np.zeros([10, 10]), 0.0, 0.01),
        (np.ones([10, 10]), np.zeros([10, 10]), 5.0, 5.01),
    ...
    @parameterized.parameters(
        (0, 0.0, 0.01), (10, 10.0, 10.01), (100, 100.0, 100.01), (101, 100.0, 100.01)
    )
    ...
    def test_compute_covariance_z_mean(self, diagonal, mean, cov):
        ...
        with self.test_session() as sess:
            ...
            self.assertBetween(np.sum((test_value - cov) ** 2), 0.0, 0.1)

    @parameterized.parameters(
        (np.ones([10, 10]), 90.0, 90.1),
        (np.zeros([10, 10]), 10.0, 10.1),
        (np.diag(np.ones(10)), 0.0, 0.1),
        (2.0 * np.diag(np.ones(10)), 10.0, 10.1),
    )
    ...
    @parameterized.parameters((0.0, -1.4190, -1.4188), (1.0, -0.92, -0.91))
    def test_gaussian_log_density(self, z_mean, target_low, target_high):
        matrix = tf.ones(1)
        with self.test_session() as sess:
            test_value = sess.run(vae.gaussian_log_density(matrix, z_mean, 0.0))[0]
            self.assertBetween(test_value, target_low, target_high)

    @parameterized.parameters(
        (1, 0.0, 0.1), (10, -82.9, -82.89)
    )  # -82.893 = (10 - 1) * ln(10000)
