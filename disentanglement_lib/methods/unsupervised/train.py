...
from disentanglement_lib.methods.unsupervised import (
    vae,
...
def train_with_gin(
    model_dir, overwrite=False, gin_config_files=None, gin_bindings=None, pytorch=True
):
...
def pbt_with_gin(model_dir, overwrite=False, gin_config_files=None, gin_bindings=None):
    """Trains a model based on the provided gin configuration.
...
def train(
    ...
    gaussian_encoder_model.export_as_tf_hub(
        model, output_shape, tpu_estimator.latest_checkpoint(), module_export_path
    )
...
def pbt(model_dir, overwrite=False, random_seed=gin.REQUIRED):
    ...
    copyfile(
        os.path.join(model_dir, "bestmodels", "model.pth".format(id)),
