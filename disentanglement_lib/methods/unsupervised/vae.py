...
def compute_gaussian_kl(z_mean, z_logvar):
    ...
    return tf.reduce_mean(
        0.5 * tf.reduce_sum(tf.square(z_mean) + tf.exp(z_logvar) - z_logvar - 1, [1]),
        name="kl_loss",
    )
...
def anneal(c_max, step, iteration_threshold):
    ...
    return tf.math.minimum(
        c_max * 1.0, c_max * 1.0 * tf.to_float(step) / iteration_threshold
    )
...
class AnnealedVAE(BaseVAE):
    ...
    def __init__(
        self, gamma=gin.REQUIRED, c_max=gin.REQUIRED, iteration_threshold=gin.REQUIRED
    ):
...
class FactorVAE(BaseVAE):
    ...
    def model_fn(self, features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            discriminator_vars = [
                var for var in all_variables if "discriminator" in var.name
            ]
            update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
            ...
            tf.summary.scalar("reconstruction_loss", reconstruction_loss)
            logging_hook = tf.train.LoggingTensorHook(
                {"loss": factor_vae_loss, "reconstruction_loss": reconstruction_loss},
                every_n_iter=50,
            )
            return tf.contrib.tpu.TPUEstimatorSpec(
                mode=mode,
            ...
            return tf.contrib.tpu.TPUEstimatorSpec(
                ...
                loss=factor_vae_loss,
                eval_metrics=(
                    make_metric_fn("reconstruction_loss", "regularizer", "kl_loss"),
...
class DIPVAE(BaseVAE):
    ...
    def __init__(
        self, lambda_od=gin.REQUIRED, lambda_d_factor=gin.REQUIRED, dip_type="i"
    ):
...
def total_correlation(z, z_mean, z_logvar):
    ...
    log_qz_prob = gaussian_log_density(
        tf.expand_dims(z, 1), tf.expand_dims(z_mean, 0), tf.expand_dims(z_logvar, 0)
    )
...
class BetaTCVAE(BaseVAE):
    ...
    def regularizer(self, kl_loss, z_mean, z_logvar, z_sampled):
        tc = (self.beta - 1.0) * total_correlation(z_sampled, z_mean, z_logvar)
        return tc + kl_loss
