...
def bernoulli_loss(
    true_images, reconstructed_images, activation, subtract_true_image_entropy=False
):
...
def l2_loss(true_images, reconstructed_images, activation):
    ...
    if activation == "logits":
        ...
        return tf.reduce_sum(tf.square(true_images - reconstructed_images), [1, 2, 3])
    else:
        raise NotImplementedError("Activation not supported.")
...
def make_reconstruction_loss(
    true_images, reconstructed_images, loss_fn=gin.REQUIRED, activation="logits"
):
