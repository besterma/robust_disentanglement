...
def visualize(
    ...
    with gin.unlock_config():
        ...
        gin.bind_parameter(
            "VAE.num_channels", int(gin_dict["VAE.num_channels"].replace("'", ""))
        )
        gin.bind_parameter(
            "VAE.use_cuda", bool(gin_dict["VAE.use_cuda"].replace("'", ""))
        )
    ...
    if not pytorch:
        ...
        with hub.eval_function_for_module(module_path) as f:
            ...
            raw_pics = f(
                dict(images=real_pics), signature="reconstructions", as_dict=True
            )["images"]
            ...
            latents = f(dict(images=obs), signature="gaussian_encoder", as_dict=True)[
                "mean"
            ]
...
def latent_traversal_1d_multi_dim(
    generator_fn, latent_vector, dimensions=None, values=None, transpose=False
):
    ...
    if dimensions is None:
        ...
        if dimensions > latent_vector.shape[0]:
            raise ValueError(
                "The number of dimensions of latent_vector is less than"
