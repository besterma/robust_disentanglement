...
def compute_factor_vae(
    ...
    global_variances = _compute_variances(
        ground_truth_data, representation_function, num_variance_estimate, random_state
    )
    active_dims = _prune_dims(global_variances)
    scores_dict = {}

    if not active_dims.any():
        scores_dict["train_accuracy"] = 0.0
        scores_dict["eval_accuracy"] = 0.0
    ...
    train_accuracy = (
        np.sum(training_votes[classifier, other_index]) * 1.0 / np.sum(training_votes)
    )
...
def _compute_variances(
    ...
    representations = utils.obtain_representation(
        observations, representation_function, eval_batch_size
    )
...
def _generate_training_batch(
    ...
    """
    votes = np.zeros(
        (ground_truth_data.num_factors, global_variances.shape[0]), dtype=np.int64
    )
