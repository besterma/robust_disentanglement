...
def unsupervised_metrics(
    ...
    scores["gaussian_wasserstein_correlation_norm"] = scores[
        "gaussian_wasserstein_correlation"
    ] / np.sum(np.diag(cov_mus))
...
def kl_gaussians_numerically_unstable(mean_0, cov_0, mean_1, cov_1, k):
    ...
    return 0.5 * (
        np.trace(np.matmul(inv_1, cov_0))
        + np.dot(mean_1 - mean_0, np.dot(inv_1, mean_1 - mean_0))
        - k
        + np.log(det_1 / det_0)
    )
