...
def compute_reduced_downstream_task(
    ground_truth_data,
    ...
    for train_size in num_train:
        ...
        for factor_of_interest in range(ground_truth_data.num_factors):
            ...
            for _ in range(num_factors_to_remove):
                reduced_mus_train, reduced_mus_test = compute_reduced_representation(
                    reduced_mus_train,
                    ys_train,
                    reduced_mus_test,
                    ys_test,
            ...
            train_acc, test_acc = compute_predictive_accuracy(
                np.transpose(reduced_mus_train),
                ys_train,
            ...
            ] = np.mean(local_other_factors_test_scores)
            other_factors_train_scores.append(np.mean(local_other_factors_train_scores))
...
def compute_reduced_representation(
    ...
    """
    importance_matrix = correlation_measure(mus_train, ys_train, mus_test, ys_test)
...
@gin.configurable(
    "factorwise_dci", blacklist=["mus_train", "ys_train", "mus_test", "ys_test"]
)
