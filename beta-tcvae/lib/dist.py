...
from beta_tcvae.lib.functions import STHeaviside
...
class Normal(nn.Module):
    ...
    def __init__(self, mu=0, sigma=1, device=0):
    ...
    def set_device(self, device):
        # print("setting device for", self, " distr to", device, ",before it was", self.device)
        self.device = device
    ...
    def _check_inputs(self, size, mu_logsigma):
        if size is None and mu_logsigma is None:
            ...
            mu = mu_logsigma.select(-1, 0).expand(size).to(device=self.device)
            logsigma = mu_logsigma.select(-1, 1).expand(size).to(device=self.device)
            return mu, logsigma
        elif size is not None:
            mu = self.mu.expand(size).to(self.device)
            logsigma = self.logsigma.expand(size).to(self.device)
            return mu, logsigma
        elif mu_logsigma is not None:
            mu = mu_logsigma.select(-1, 0).to(device=self.device)
            logsigma = mu_logsigma.select(-1, 1).to(device=self.device)
            return mu, logsigma
        else:
            raise ValueError(
    ...
    def sample(self, size=None, params=None):
        # print("putting std_z on gpu", self.device)
        mu, logsigma = self._check_inputs(size, params)
        std_z = Variable(torch.randn(mu.size()).type_as(mu.data)).to(device=self.device)
    ...
    def log_density(self, sample, params=None):
        ...
        c = self.normalization.type_as(sample.data).to(device=self.device)
...
class Laplace(nn.Module):
    ...
    def __init__(self, mu=0, scale=1, device=0):
    ...
    def sample(self, size=None, params=None):
        ...
        # Unif(-0.5, 0.5)
        u = (
            Variable(torch.rand(mu.size()).type_as(mu.data)).to(device=self.device)
            - 0.5
        )
...
class Bernoulli(nn.Module):
    ...
    def __init__(self, p=0.5, stgradient=False, device=0):
    ...
    def _sample_logistic(self, size):
        u = Variable(torch.rand(size)).to(device=self.device)
