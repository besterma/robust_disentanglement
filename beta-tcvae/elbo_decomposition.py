...
import beta_tcvae.lib.flows as flows


@torch.no_grad()
def estimate_entropies(qz_samples, qz_params, q_dist, device="cpu", n_samples=1000):
    """Computes the term:
    ...
    qz_samples = qz_samples.to(device)
...
@torch.no_grad()
def elbo_decomposition(vae, dataset_loader, device="cpu"):
    with torch.cuda.device(device):
        ...
        for xs in dataset_loader:
            ...
            xs = Variable(xs.view(batch_size, -1, 64, 64).cuda()).to(device)
            z_params = vae.encoder.forward(xs).view(batch_size, K, nparams).to(device)
            qz_params[n : n + batch_size] = z_params.data
            n += batch_size

            # estimate reconstruction term
            for _ in range(S):
                z = vae.q_dist.sample(params=z_params).to(device)
        ...
        qz_params = Variable(qz_params.to(device))

        print("Sampling from q(z).")
        # sample S times from each marginal q(z_j|x_n)
        qz_params_expanded = (
            qz_params.view(N, K, 1, nparams).expand(N, K, S, nparams).to(device)
        )
        qz_samples = vae.q_dist.sample(params=qz_params_expanded).to(device)
        ...
        marginal_entropies, joint_entropy = estimate_entropies(
            qz_samples, qz_params, vae.q_dist, device
        )
